...
const LichSuThuoc = () => {
    ...
    useEffect(() => {
        const fetchLichSu = async () => {
            try {
                const response = await axios.get('http://localhost:3000/admin/lichsuthuoc');
    ...
    if (!lichSu || lichSu.length === 0) {
        return (
            <div className="bg-white mx-10 p-8">
                <h3 className="text-3xl font-bold text-[#22223B] mb-6">Lịch sử cập nhật thuốc</h3>
                <div className="text-center py-8 text-gray-500">
                    <p className="text-lg">Chưa có lịch sử chỉnh sửa thuốc</p>
                </div>
            </div>
        );
    }
